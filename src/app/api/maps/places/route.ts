...
const placesCache = new Map<string, { expiresAt: number; predictions: Array<{
  placeId: string; primaryText: string; secondaryText: string; fullText: string;
}> }>();
...
export async function GET(request: Request) {
  ...
  const params = new URLSearchParams({
    input, key: apiKey, components: "country:in",
    types: "geocode|establishment", language: "en",
  });
  ...
  if (!res.ok) {
    return NextResponse.json({ ok: false, message: "Google Places API error" }, { status: 502 });
  }
  ...
  placesCache.set(cacheKey, { predictions, expiresAt: Date.now() + PLACES_CACHE_TTL_MS });
  return NextResponse.json({ ok: true, data: { predictions } });
}
