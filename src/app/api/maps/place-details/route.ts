...
import {
  requireMapsActor, getGoogleMapsApiKey, missingKeyResponse, enforceMapsRateLimit,
...
function extractCity(components: AddressComponent[]): string {
  for (const c of components) if (c.types.includes("locality")) return c.long_name;
  for (const c of components) if (c.types.includes("administrative_area_level_2")) return c.long_name;
  for (const c of components) if (c.types.includes("sublocality_level_1")) return c.long_name;
  return "";
}

function extractState(components: AddressComponent[]): string | null {
  for (const c of components) if (c.types.includes("administrative_area_level_1")) return c.long_name;
  return null;
}
...
export async function GET(request: Request) {
  ...
  if (!placeId) {
    return NextResponse.json({ ok: false, message: "placeId is required" }, { status: 400 });
  }
  if (placeId.length > MAX_PLACE_ID_LENGTH || !PLACE_ID_RE.test(placeId)) {
    return NextResponse.json({ ok: false, message: "Invalid placeId" }, { status: 400 });
  }
  ...
  if (!res.ok) {
    return NextResponse.json({ ok: false, message: "Google Place Details API error" }, { status: 502 });
  }

  const body = (await res.json()) as {
    status: string;
    result?: {
      place_id: string; formatted_address: string; name?: string;
  ...
  placeDetailsCache.set(cacheKey, { data, expiresAt: Date.now() + PLACE_DETAILS_CACHE_TTL_MS });
